                                ...
                                <tbody>
                                    @if (@$data->isNotEmpty())
                                        @foreach (@$data as $att)
                                            <tr>
                                                <td>{{ @$att->name}}</td>
                                                <td>{{ @$att->department_name->name }}</td>
                                                ...
                                                <td>
                                                    ...
                                                    <a type="button"
                                                        class="btn btn-xs btn-primary edit_button row-class-{{ @$att->dzoID }}"
                                                         data-name="{{$att->name}}" data-department="{{$att->department_id}}"  data-id="{{$att->id}}"   data-toggle="modal"
                                                        >
                                                        Edit
                                                    </a>
                                                    @endif
                                                    ...
                                                    <a class="btn btn-xs btn-danger"
                                                        href="{{ route('manage.division.delete', ['id' => @$att->id]) }}"
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form method="post" action="{{ route('manage.division.add') }}">@csrf
            ...
            <div class="modal fade" id="exampleModaEdit" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form method="post" action="{{ route('manage.division.update') }}">@csrf
                                ...
                                <div class="form-group">
                                    <label for="exampleInputEmail1">Division Name</label>
                                    <input type="text" class="form-control"  name="name"
                                         placeholder="Division Name" id="name" required>
                                </div>
...
// ;
        ...
        $('.edit_button').on('click',function(){
            ...
            var dropdown = $(this).data('department');
            $('#department  option[value="'+dropdown+'"]').prop("selected", true);
